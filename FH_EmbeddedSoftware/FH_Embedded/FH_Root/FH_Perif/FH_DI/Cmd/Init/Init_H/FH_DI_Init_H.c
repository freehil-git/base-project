/**
  ...
  * @brief
  *       <b>File Map:</b>\n
*       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_DI</b> (Digital Input), <b>Cmd</b> (Command), <b>Init</b>, <b>Init_H</b> (Init Health)\n
...
/**
 * @brief This function verifies the health of <b>FH_DI_Commands_Init</b> command
 *
...
*/
FH_ErrorInfo FH_DI_Init_H(FH_RFCommunication_Message* fh_RFCommunication_Message)
{
	...
	if (FH_MessageDataLength < FH_DI_MinCommandSize_Init)    // If the length is less than expected value
	{
	...
	if (FH_PeripheralNumber == 0 || FH_PeripheralNumber > FH_DI_MaxPeripheralNumber)    // If the PeripheralNumber is not within the expected value
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}
...
/**
 * @brief This function verifies the health of <b>FH_DI_Commands_InitMULTI</b> command
 *
 * @param  fh_RFCommunication_Message
 *         The message frame set by <b>RobotFramework</b>
 *
 * @verbatim
    ...
    01    02    03    04    05    06    07    08 => Example data as the first 8 parameters
    DeviceAddress_01 => It shall be equal to the address of the device: FH_RFCommunication_DeviceAddress
    Function_DI => It is the function in the message frame
...
*/
FH_ErrorInfo FH_DI_InitMULTI_H(FH_RFCommunication_Message* fh_RFCommunication_Message)
{
	FH_ErrorInfo fh_ErrorInfo;    // Error Information
	FH_ResetErrorInfo(&fh_ErrorInfo);    // Reset Error Information to default
	...
	// Verifying the minimum possible length in the message frame
	FH_MessageDataLength =	fh_RFCommunication_Message -> DataLength;
	if (FH_MessageDataLength < FH_DI_MinCommandSize_InitMULTI)    // If the length is less than expected value
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}
