...
FH_ErrorInfo FH_DI_GetStatus_H(FH_RFCommunication_Message* fh_RFCommunication_Message)
{
	...
	if (FH_MessageDataLength < FH_DI_MinCommandSize_GetStatus)    // If the length is less than expected value
	{
...
FH_ErrorInfo FH_DI_GetStatusMULTI_H(FH_RFCommunication_Message* fh_RFCommunication_Message)
{
	...
	// Verifying the minimum possible length in the message frame
	FH_MessageDataLength =	fh_RFCommunication_Message -> DataLength;
	if (FH_MessageDataLength < FH_DI_MinCommandSize_GetStatusMULTI)    // If the length is less than expected value
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}
