/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_UART</b>, <b>Cmd</b> (Command), <b>Receive</b>, <b>Receive_C</b> (Receive Call)\n
...
*/
FH_ErrorInfo FH_UART_Receive_C (FH_RFCommunication_Message* fh_RFCommunication_Message, uint8_t* fH_UART_Data, uint8_t* fH_UART_DataSize, FH_UART_Receive_FuncPtrModel *fH_UART_Receive_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[UART_Receive_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_UART_Receive_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_UART_Receive_FuncPtr[i-1](fh_RFCommunication_Message, fH_UART_DataSize, fH_UART_Data);    // Call the related function
			}
			else
			{
