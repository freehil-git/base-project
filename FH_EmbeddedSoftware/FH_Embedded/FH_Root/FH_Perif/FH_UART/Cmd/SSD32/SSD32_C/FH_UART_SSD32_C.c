...
FH_ErrorInfo FH_UART_SSD32_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_UART_SSD32_FuncPtrModel *FH_UART_SSD32_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[UART_SSD32_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (FH_UART_SSD32_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = FH_UART_SSD32_FuncPtr[i-1](fh_RFCommunication_Message);    // Call the related function
			}
			else
			{
