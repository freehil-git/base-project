...
FH_ErrorInfo FH_UART_GSD16_C (FH_RFCommunication_Message* fh_RFCommunication_Message, uint8_t* SharedDataLength, uint8_t* SharedData, FH_UART_GSD16_FuncPtrModel *fH_UART_GSD16_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[UART_GSD16_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_UART_GSD16_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_UART_GSD16_FuncPtr[i-1](fh_RFCommunication_Message, SharedDataLength, SharedData);    // Call the related function
			}
			else
			{
