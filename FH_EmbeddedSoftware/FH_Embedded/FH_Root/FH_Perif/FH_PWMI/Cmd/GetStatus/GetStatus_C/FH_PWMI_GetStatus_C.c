 ...
 * @param  fH_PWMI_Data
 *         *fH_PWMI_Data will be set with the value of the related <b>PWM Input</b>
...
FH_ErrorInfo FH_PWMI_GetStatus_C (FH_RFCommunication_Message* fh_RFCommunication_Message, uint32_t* fH_PWMI_Frequecncy, uint8_t* fH_PWMI_DutyCycle, FH_PWMI_GetStatus_FuncPtrModel *fH_PWMI_GetStatus_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[PWMI_GetStatus_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_PWMI_GetStatus_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_PWMI_GetStatus_FuncPtr[i-1](fh_RFCommunication_Message, fH_PWMI_Frequecncy, fH_PWMI_DutyCycle);    // Call the related function
			}
			else
			{
