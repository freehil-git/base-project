/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_PWMI</b> (PWM Input), <b>Cmd</b> (Command), <b>Init</b>, <b>Init_C</b> (Init Call)\n
...
/**
 * @brief This function calls the related function in <b>FH_Functions</b>, Section <b>FH_PWMI</b> (PWM Input) according to the first parameter of the Data in the message frame
 *
...
*/
FH_ErrorInfo FH_PWMI_Init_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_PWMI_Init_FuncPtrModel *fH_PWMI_Init_FuncPtr)
{
	FH_ErrorInfo fh_ErrorInfo;    // Error Information
	FH_ResetErrorInfo(&fh_ErrorInfo);    // Reset Error Information to default

	for (uint8_t i = 1; i <= FH_PWMI_MaxPeripheralNumber; i++)    // Iteration through all defined  peripherals
	{
		if (i == fh_RFCommunication_Message -> Data[PWMI_Init_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_PWMI_Init_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_PWMI_Init_FuncPtr[i-1](fh_RFCommunication_Message);    // Call the related function
			}
			else
			{
