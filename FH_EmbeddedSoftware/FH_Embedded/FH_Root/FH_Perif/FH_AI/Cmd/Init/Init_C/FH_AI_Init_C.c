/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_AI</b> (Analog Input), <b>Cmd</b> (Command), <b>Init</b>, <b>Init_C</b> (Init Call)\n
...
FH_ErrorInfo FH_AI_Init_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_AI_Init_FuncPtrModel *fH_AI_Init_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[AI_Init_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_AI_Init_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_AI_Init_FuncPtr[i-1](fh_RFCommunication_Message);    // Call the related function
			}
			else
			{
