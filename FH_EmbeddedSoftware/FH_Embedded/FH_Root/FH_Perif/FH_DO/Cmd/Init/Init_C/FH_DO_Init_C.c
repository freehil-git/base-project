/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_DO</b> (Digital Output), <b>Cmd</b> (Command), <b>Init</b>, <b>Init_C</b> (Init Call)\n
  *       Functions in this file, Call the related function in <b>FH_Functions</b>, Section <b>FH_DO</b> (Digital Output) according to the first parameter of the Data in the message frame
  *
...
FH_ErrorInfo FH_DO_Init_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_DO_Init_FuncPtrModel *FH_DO_Init_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[DO_Init_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (FH_DO_Init_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = FH_DO_Init_FuncPtr[i-1]();    // Call the related function
			}
			else
			{
 ...
 *
 * @warning
 *	   By default, in <b>InitMULTI</b> (Which is mass initialization of up to 64 <b>DO</b> (Digital Output)) command, 8 bytes which is 64 bits (which means 64 <b>DO</b> (Digital Output)) is used in the command\n
 *     To use <b>InitMULTI</b> (Which is mass initialization of up to 64 <b>DO</b> (Digital Output)) command, <b>FH_DO_MaxPeripheralNumber</b> and <b>FH_DO_MULTI_MaxPeripheralNumberLimit</b> shall be set to 64
 *
...
FH_ErrorInfo FH_DO_InitMULTI_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_DO_Init_FuncPtrModel *FH_DO_Init_FuncPtr)
{
	...
	uint8_t DO_InitMULTI_MaskByteCounter = 0;    // It temporarily stores the index (0 - 7) of the 8 bytes of mask in the Data part of the message frame
    uint8_t DO_InitMULTI_MaskByte = 0;    // It temporarily stores the byte value with the index number (DO_InitMULTI_MaskByteCounter) of the 8 bytes of mask in the Data part of the message frame
	uint8_t DO_InitMULTI_MaskBitCounter = 0;    // It temporarily stores the index (0 - 7) of the every bit in the byte of mask in the Data part of the message frame
	uint8_t DO_InitMULTI_MaskBit = 0;    // It temporarily stores the bit value with the index number (DO_InitMULTI_MaskBitCounter) of every bit in the byte of mask in the Data part of the message frame
	...
	{
		MaxPeripheralNumber = FH_DO_MaxPeripheralNumber;    // The minimum is FH_DI_MaxPeripheralNumber
	}

	for (uint8_t i = 1; i <= MaxPeripheralNumber; i++)    // Iteration through all defined  peripherals
	{
		DO_InitMULTI_MaskByte = fh_RFCommunication_Message -> Data[DO_InitMULTI_MaskByteCounter];    // Temporarily store the related byte value with the related index number of the 8 bytes of mask
		DO_InitMULTI_MaskBit  = DO_InitMULTI_MaskByte & (0x80 >> DO_InitMULTI_MaskBitCounter);    // Temporarily store the related bit value from the related byte value from the 8 bytes of mask
		if (DO_InitMULTI_MaskBit != 0)    // If the bit is 1
		{
			if (FH_DO_Init_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				FH_DO_Init_FuncPtr[i-1]();    // Call the related function
			}
			else
			{
			}
		}
		DO_InitMULTI_MaskBitCounter++;    // Increase the DO_InitMULTI_MaskBitCounter
		if (DO_InitMULTI_MaskBitCounter == 8)    // If the whole bits of the byte are already processed
		{
			DO_InitMULTI_MaskBitCounter = 0;    // Reset DO_InitMULTI_MaskBitCounter
			DO_InitMULTI_MaskByteCounter++;    // Increase DO_InitMULTI_MaskByteCounter
		}
	}
