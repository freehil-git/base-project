/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_CAN</b>, <b>Cmd</b> (Command), <b>ReceiveW</b>, <b>ReceiveW_C</b> (ReceiveW Call)\n
...
/**
 * @brief This function calls the related function in <b>FH_Functions</b>, Section <b>FH_CAN</b> according to the first parameter of the Data in the message frame
 *
 * @param  fh_RFCommunication_Message
 *         The message frame set by <b>RobotFramework</b>
 * @param  fH_CAN_Data
 *         *fH_CAN_Data will be set with the related ReceiveBuf
 * @param  FH_CAN_DataSize
 *         *FH_CAN_DataSize will be set with the length of the related ReceiveBuf
...
*/
FH_ErrorInfo FH_CAN_ReceiveW_C (FH_RFCommunication_Message* fh_RFCommunication_Message, uint8_t* fH_CAN_Data, uint8_t* fH_CAN_DataSize, FH_CAN_ReceiveW_FuncPtrModel *fH_CAN_ReceiveW_FuncPtr)
{
	FH_ErrorInfo fh_ErrorInfo;    // Error Information
	FH_ResetErrorInfo(&fh_ErrorInfo);    // Reset Error Information to default

	for (uint8_t i = 1; i <= FH_CAN_MaxPeripheralNumber; i++)    // Iteration through all defined  peripherals
	{
		if (i == fh_RFCommunication_Message -> Data[CAN_ReceiveW_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_CAN_ReceiveW_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_CAN_ReceiveW_FuncPtr[i-1](fh_RFCommunication_Message, fH_CAN_DataSize, fH_CAN_Data);    // Call the related function
			}
			else
			{
