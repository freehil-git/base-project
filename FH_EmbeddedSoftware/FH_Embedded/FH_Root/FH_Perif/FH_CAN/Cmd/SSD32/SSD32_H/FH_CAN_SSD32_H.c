/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_CAN</b>, <b>Cmd</b> (Command), <b>SSD32</b>, <b>SSD32_H</b> (Set Shared Data 32 Health)\n
...
/**
 * @brief This function verifies the health of <b>FH_CAN_Commands_SSD32</b> command
 *
...
*/
FH_ErrorInfo FH_CAN_SSD32_H(FH_RFCommunication_Message* fh_RFCommunication_Message)
{
	...
	uint8_t FH_SharedDataLength;    // It temporarily stores the SharedDataLength (Data[1]) of the 32 bit data to be set

	// Verifying the minimum possible length in the message frame
	FH_MessageDataLength =  fh_RFCommunication_Message -> DataLength;
	if (FH_MessageDataLength < FH_CAN_MinCommandSize_SSD32)    // If the length is less than expected value
	{
	...
	FH_PeripheralNumber = fh_RFCommunication_Message -> Data[CAN_SSD32_PeripheralNumber];
	if (FH_PeripheralNumber == 0 || FH_PeripheralNumber > FH_CAN_MaxPeripheralNumber)    // If the PeripheralNumber is not within the expected value
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}

    // Verifying the possible SharedDataLength in the message frame
	FH_SharedDataLength  = fh_RFCommunication_Message -> Data[CAN_SSD32_SharedDataLength];
	if (FH_SharedDataLength == 0)    // If SharedDataLength is 0
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}

	if ((FH_SharedDataLength * 4) != (FH_MessageDataLength - 2))    // If SharedDataLength is not matched with the length of the message frame
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}
