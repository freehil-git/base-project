...
FH_ErrorInfo FH_CAN_ResetRB_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_CAN_ResetRB_FuncPtrModel *fH_CAN_ResetRB_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[CAN_ResetRB_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_CAN_ResetRB_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_CAN_ResetRB_FuncPtr[i-1](fh_RFCommunication_Message);    // Call the related function
			}
			else
			{
