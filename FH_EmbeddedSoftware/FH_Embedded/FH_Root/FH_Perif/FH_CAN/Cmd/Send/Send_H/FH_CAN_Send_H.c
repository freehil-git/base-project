/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_CAN</b>, <b>Cmd</b> (Command), <b>Send</b>, <b>Send_H</b> (Send Health)\n
...
/**
 * @brief This function verifies the health of <b>FH_CAN_Commands_Send</b> command
 *
...
FH_ErrorInfo FH_CAN_Send_H(FH_RFCommunication_Message* fh_RFCommunication_Message)
{
	...
	uint8_t FH_DataLength;    // It temporarily stores the DataLength (Data[1]) of the data supposed to be sent

	// Verifying the minimum possible length in the message frame
	FH_MessageDataLength =  fh_RFCommunication_Message -> DataLength;
	if (FH_MessageDataLength < FH_CAN_MinCommandSize_Send)    // If the length is less than expected value
	{
	...
	FH_PeripheralNumber = fh_RFCommunication_Message -> Data[CAN_Send_PeripheralNumber] ;
	...
	// Verifying the possible DataLength of the data supposed to be sent
	FH_DataLength = fh_RFCommunication_Message -> Data[CAN_Send_DataLength] ;
	if (FH_DataLength == 0)    // If the DataLength is 0
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}

	if (FH_DataLength != (FH_MessageDataLength - 2))    // If SharedDataLength is not matched with the length of the message frame
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}
