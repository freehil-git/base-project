/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_CAN</b>, <b>Cmd</b> (Command), <b>Send</b>, <b>Send_C</b> (Send Call)\n
...
FH_ErrorInfo FH_CAN_Send_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_CAN_Send_FuncPtrModel *fH_CAN_Send_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[CAN_Send_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (fH_CAN_Send_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = fH_CAN_Send_FuncPtr[i-1](fh_RFCommunication_Message);    // Call the related function
			}
			else
			{
