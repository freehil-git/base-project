/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_CAN</b>, <b>Cmd</b> (Command), <b>SSD16</b>, <b>SSD16_H</b> (Set Shared Data 16 Health)\n
...
/**
 * @brief This function verifies the health of <b>FH_CAN_Commands_SSD16</b> command
 *
...
*/
FH_ErrorInfo FH_CAN_SSD16_H(FH_RFCommunication_Message* fh_RFCommunication_Message)
{
	...
	uint8_t FH_SharedDataLength;    // It temporarily stores the SharedDataLength (Data[1]) of the 16 bit data to be set

	// Verifying the minimum possible length in the message frame
	FH_MessageDataLength =  fh_RFCommunication_Message -> DataLength;
	if (FH_MessageDataLength < FH_CAN_MinCommandSize_SSD16)    // If the length is less than expected value
	{
	...
	FH_PeripheralNumber = fh_RFCommunication_Message -> Data[CAN_SSD16_PeripheralNumber] ;
	...
	FH_SharedDataLength  = fh_RFCommunication_Message -> Data[CAN_SSD16_SharedDataLength];
	...
	if ((FH_SharedDataLength * 2) != (FH_MessageDataLength - 2))    // If SharedDataLength is not matched with the length of the message frame
	{
		fh_ErrorInfo.error_code = FH_ERROR_MessageFrame;    // There is message frame error
		return fh_ErrorInfo;
	}
