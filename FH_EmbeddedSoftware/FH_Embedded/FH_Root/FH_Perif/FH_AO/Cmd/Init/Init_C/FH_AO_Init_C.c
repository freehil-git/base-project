/**
  ...
  * @brief
  *       <b>File Map:</b>\n
  *       <b>FH_Root</b>, Section <b>FH_Perif</b>, <b>FH_AO</b> (Analog Output), <b>Cmd</b> (Command), <b>Init</b>, <b>Init_C</b> (Init Call)\n
...
FH_ErrorInfo FH_AO_Init_C (FH_RFCommunication_Message* fh_RFCommunication_Message, FH_AO_Init_FuncPtrModel *FH_AO_Init_FuncPtr)
{
	...
	{
		if (i == fh_RFCommunication_Message -> Data[AO_Init_PeripheralNumber])    // If the PeripheralNumber in the message frame is met
		{
			if (FH_AO_Init_FuncPtr[i-1] != NULL_PTR)    // If the address of the function is truly assigned
			{
				fh_ErrorInfo = FH_AO_Init_FuncPtr[i-1](fh_RFCommunication_Message);    // Call the related function
			}
			else
			{
